(function(){
	...
	suite("my-app", function(){
		...
		test("ensure sum works with expected data", function(){
			...
			assert.strictEqual( sum([1,0,3]), 4, "sum works with array with a 0 numbers");
			assert.strictEqual(sum([]), 0, "sum works with no array entries");
			assert.strictEqual(sum([3]), 3, "sum works with 1 entry");

		}); 

		test("ensure sum works with unexpected data args", function(){

			assert.strictEqual( sum(), 0, "no args results in zero sum");
			assert.ok( isNaN(sum("foo")), "bad arg results in NaN" );
			assert.ok( isNaN(sum(["a","b","c"])), "non-numeric array entries result in NaN" );
		});
